...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    if KATYA_ID and str(message.from_user.id) == str(KATYA_ID):
        ...
        await message.answer(
            f"Привет, {username}! 👋\n\n"
            "Добро пожаловать в BezSkolov Bot - бот для подбора краски и заказа минитюбиков с заводской краской для закрашивания сколов!\n\n"
            "Выберите действие:",
...
async def change_request_status(callback: types.CallbackQuery, bot: Bot):
    ...
    try:
        status_messages = {
            ...
            "completed": f"🟢 Ваша заявка #{request_id} выполнена!\n\nМинитюбик с краской {request.brand} {request.model} готов к выдаче. Спасибо за обращение!",
